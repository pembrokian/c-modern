...
func validate_state_hardening_contracts(audit: StateHardeningAudit) bool {
    ...
    append15: state.BootLogAppendResult = state.append_record(append14.log, state.BootStage.Halted, audit.arch_actor, 16)
    overflow: state.BootLogAppendResult = state.append_record(append15.log, state.BootStage.Halted, audit.arch_actor, 17)
    if overflow.appended != 0 {
        return false
    }
    return overflow.log.count == 16
}
