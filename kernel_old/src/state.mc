...
func task_identity(pid: u32, tid: u32, asid: u32) TaskIdentity {
    return TaskIdentity{ pid: pid, tid: tid, asid: asid }
}

func identity_config(boot: TaskIdentity, init: TaskIdentity, child: TaskIdentity) IdentityConfig {
    return IdentityConfig{ boot: boot, init: init, child: child }
}

func empty_contract_state() ContractState {
    return ContractState{ scheduler: 0, lifecycle: 0, capability: 0, ipc: 0, address_space: 0, interrupt: 0, timer: 0, barrier: 0 }
}

func contract_state(scheduler: u32, lifecycle: u32, capability: u32, ipc: u32, address_space: u32, interrupt: u32, timer: u32, barrier: u32) ContractState {
    return ContractState{ scheduler: scheduler, lifecycle: lifecycle, capability: capability, ipc: ipc, address_space: address_space, interrupt: interrupt, timer: timer, barrier: barrier }
}
...
func zero_boot_records() [16]BootRecord {
    records: [16]BootRecord
    ...
    records[14] = empty_record()
    records[15] = BootRecord{ stage: BootStage.Reset, actor: 0, detail: 0 }
    return records
}
...
func append_record(log: BootLog, stage: BootStage, actor: u32, detail: u32) BootLogAppendResult {
    ...
    entries: [16]BootRecord = log.entries
