...
func empty_space() AddressSpace {
    return AddressSpace{ asid: 0, owner_pid: 0, state: AddressSpaceState.Empty, translation_root: mmu.empty_translation_root(), entry_pc: 0, stack_top: 0, mapping_count: 0, mappings: zero_mappings() }
}
...
func bootstrap_space_valid(translation_root: mmu.TranslationRoot, image_base: usize, image_size: usize, entry_pc: usize, stack_base: usize, stack_size: usize, stack_top: usize) bool {
    if mmu.state_score(translation_root.state) != 2 {
        return false
    }
    return bootstrap_layout_valid(image_base, image_size, entry_pc, stack_base, stack_size, stack_top)
}

func bootstrap_space(asid: u32, owner_pid: u32, translation_root: mmu.TranslationRoot, image_base: usize, image_size: usize, entry_pc: usize, stack_base: usize, stack_size: usize, stack_top: usize) AddressSpace {
    if !bootstrap_space_valid(translation_root, image_base, image_size, entry_pc, stack_base, stack_size, stack_top) {
        return empty_space()
    }
    ...
    return AddressSpace{ asid: asid, owner_pid: owner_pid, state: AddressSpaceState.Constructed, translation_root: translation_root, entry_pc: entry_pc, stack_top: stack_top, mapping_count: 2, mappings: mappings }
}
...
func can_activate(space: AddressSpace) bool {
    ...
    if !mmu.can_activate(space.translation_root) {
        return false
    }
...
func activate(space: AddressSpace) AddressSpace {
    ...
    return AddressSpace{ asid: space.asid, owner_pid: space.owner_pid, state: AddressSpaceState.Active, translation_root: mmu.activate(space.translation_root), entry_pc: space.entry_pc, stack_top: space.stack_top, mapping_count: space.mapping_count, mappings: space.mappings }
}
...
func build_child_bootstrap_context(owner_pid: u32, child_tid: u32, child_asid: u32, translation_root: mmu.TranslationRoot, image_base: usize, image_size: usize, entry_pc: usize, stack_base: usize, stack_size: usize, stack_top: usize) SpawnBootstrap {
    child_space: AddressSpace = bootstrap_space(child_asid, owner_pid, translation_root, image_base, image_size, entry_pc, stack_base, stack_size, stack_top)
...
func validate_syscall_address_space_boundary() bool {
    child: SpawnBootstrap = build_child_bootstrap_context(3, 4, 5, mmu.bootstrap_translation_root(5, 32768), 65536, 8192, 66048, 131072, 8192, 139264)
    if child.valid == 0 {
        return false
    }
    ...
    if child.child_address_space.translation_root.page_table_base != 32768 {
        return false
    }
    return true
}
