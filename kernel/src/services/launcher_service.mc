...
func launcher_list(s: LauncherServiceState) LauncherResult {
    first := program_catalog.program_descriptor_at(0)
    second := program_catalog.program_descriptor_at(1)
    return LauncherResult{
        state: s,
        effect: launcher_reply(syscall.SyscallStatus.Ok, 4, u8(program_catalog.program_count()), first.id, second.id, s.selected)
    }
}

func launcher_select(s: LauncherServiceState, id: u8) LauncherResult {
    desc := program_catalog.program_descriptor_for_id(id)
    if !program_catalog.program_descriptor_is_valid(desc) {
        return LauncherResult{ state: s, effect: launcher_reply(syscall.SyscallStatus.InvalidArgument, 0, 0, 0, 0, 0) }
    }
    next := launcherwith(s, id, s.foreground, s.launches)
    return LauncherResult{ state: next, effect: launcher_reply(syscall.SyscallStatus.Ok, 2, next.selected, next.foreground, 0, 0) }
}

func launcher_launch(s: LauncherServiceState) LauncherResult {
    ...
    return LauncherResult{
        state: next,
        effect: launcher_reply(syscall.SyscallStatus.Ok, 3, next.foreground, program_catalog.program_launch_kind_code(desc), next.launches, 0)
    }
}

func handle(s: LauncherServiceState, m: service_effect.Message) LauncherResult {
    if m.payload_len == 0 {
        return LauncherResult{ state: s, effect: launcher_reply(syscall.SyscallStatus.InvalidArgument, 0, 0, 0, 0, 0) }
    }
    ...
    case LAUNCHER_OP_LIST:
        ...
        return launcher_list(s)
    case LAUNCHER_OP_SELECT:
        if m.payload_len != 2 {
            return LauncherResult{ state: s, effect: launcher_reply(syscall.SyscallStatus.InvalidArgument, 0, 0, 0, 0, 0) }
        }
        return launcher_select(s, m.payload[1])
    case LAUNCHER_OP_LAUNCH:
        if m.payload_len != 1 {
            return LauncherResult{ state: s, effect: launcher_reply(syscall.SyscallStatus.InvalidArgument, 0, 0, 0, 0, 0) }
        }
        return launcher_launch(s)
    default:
        return LauncherResult{ state: s, effect: launcher_reply(syscall.SyscallStatus.InvalidArgument, 0, 0, 0, 0, 0) }
    }
}
