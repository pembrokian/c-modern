...
func run_display_surface_probe() i32 {
    ...
    effect = kernel_dispatch.kernel_dispatch_step(&state, display_query_obs())
    if !expect_reply(effect, syscall.SyscallStatus.Ok, 4, 73, 82, 85, 80) {
    ...
    if !expect_reply(effect, syscall.SyscallStatus.Ok, 2, serial_protocol.TARGET_DISPLAY, serial_protocol.LIFECYCLE_RESET, 0, 0) {
