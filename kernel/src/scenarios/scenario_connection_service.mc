...
func run_connection_service_probe() i32 {
    ...
    if !scenario_assert.expect_reply(effect, syscall.SyscallStatus.Ok, 2, 1, connection_service.CONNECTION_STATE_OPEN, 0, 0) {
    ...
    if !scenario_assert.expect_reply(effect, syscall.SyscallStatus.Ok, 3, 1, 44, connection_service.CONNECTION_STATE_RECEIVED, 0) {
    ...
    if !scenario_assert.expect_reply(effect, syscall.SyscallStatus.Ok, 4, 1, 44, 99, connection_service.CONNECTION_STATE_REPLIED) {
    ...
    if !scenario_assert.expect_reply(effect, syscall.SyscallStatus.Ok, 2, 1, connection_service.CONNECTION_STATE_DISCONNECTED, 0, 0) {
    ...
    if !scenario_assert.expect_reply(effect, syscall.SyscallStatus.Closed, 1, connection_service.CONNECTION_STATE_DISCONNECTED, 0, 0, 0) {
    ...
    if !scenario_assert.expect_reply(effect, syscall.SyscallStatus.WouldBlock, 1, connection_service.CONNECTION_STATE_TIMED_OUT, 0, 0, 0) {
        return FAIL_CONNECTION_TIMEOUT
    }
