...
func run_surface_composition_probe() i32 {
    ...
    if !expect_display(effect, DISPLAY_STATE_STEADY_CUSTOM) {
    ...
    effect = kernel_dispatch.kernel_dispatch_step(&state, display_query_obs())
    if !expect_display(effect, DISPLAY_STATE_BUSY_CUSTOM) {
    ...
    effect = kernel_dispatch.kernel_dispatch_step(&state, display_query_obs())
    if !expect_display(effect, DISPLAY_STATE_ATTN_CUSTOM) {
    ...
    effect = kernel_dispatch.kernel_dispatch_step(&state, display_query_obs())
    if !expect_display(effect, DISPLAY_STATE_EMPTY_CUSTOM) {
