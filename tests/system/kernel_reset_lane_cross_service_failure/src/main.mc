...
func smoke_kv_write_ok_masks_advisory_log_drop() bool {
    ...
    // KS <key=7> <val=42>
    effect = boot.kernel_dispatch_step(&state, obs(75, 83, 7, 42))
    if service_effect.effect_reply_status(effect) != syscall.SyscallStatus.Ok {
        return false
    }
...
func smoke_exhausted_is_consistent_across_services() bool {
    ...
    effect = boot.kernel_dispatch_step(&state, obs(76, 65, 11, 33))
    if service_effect.effect_reply_status(effect) != syscall.SyscallStatus.Ok {
        return false
    }
    effect = boot.kernel_dispatch_step(&state, obs(76, 65, 12, 33))
    if service_effect.effect_reply_status(effect) != syscall.SyscallStatus.Ok {
        return false
    }
    effect = boot.kernel_dispatch_step(&state, obs(76, 65, 13, 33))
    if service_effect.effect_reply_status(effect) != syscall.SyscallStatus.Ok {
        return false
    }

    // Fifth log append: Exhausted.
    effect = boot.kernel_dispatch_step(&state, obs(76, 65, 14, 33))
    if service_effect.effect_reply_status(effect) != syscall.SyscallStatus.Exhausted {
        return false
    }
...
func smoke_invalid_argument_is_consistent_across_services() bool {
    ...
    // KG <key=99> !
    effect = boot.kernel_dispatch_step(&state, obs(75, 71, 99, 33))
    ...
    effect = boot.kernel_dispatch_step(&state, obs(0, 0, 0, 0))
    if service_effect.effect_reply_status(effect) != syscall.SyscallStatus.InvalidArgument {
        return false
    }
...
func smoke_kv_overwrite_ok_masks_advisory_log_drop() bool {
    ...
    // Write key 6 = 10.
    effect = boot.kernel_dispatch_step(&state, obs(75, 83, 6, 10))
    if service_effect.effect_reply_status(effect) != syscall.SyscallStatus.Ok {
        return false
    }
    ...
    effect = boot.kernel_dispatch_step(&state, obs(75, 83, 6, 99))
    if service_effect.effect_reply_status(effect) != syscall.SyscallStatus.Ok {
        return false
    }

    // Confirm the overwrite took effect: KG 6 should return 99.
    effect = boot.kernel_dispatch_step(&state, obs(75, 71, 6, 33))
    if service_effect.effect_reply_status(effect) != syscall.SyscallStatus.Ok {
        return false
    }
