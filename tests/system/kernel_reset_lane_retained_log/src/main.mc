...
func smoke_retains_appended_bytes() bool {
    ...
    reply: service_effect.Effect

    first_payload[0] = 65
    second_payload[0] = 66

    state = log_service.log_append(state, build_receive_observation(7, 90, 1, first_payload))
    state = log_service.log_append(state, build_receive_observation(7, 90, 1, second_payload))
    if log_service.log_len(state) != 2 {
        return false
    }

    reply = log_service.log_tail(state)
    ...
    reply_payload: [4]u8 = service_effect.effect_reply_payload(reply)
    if reply_payload[0] != 65 || reply_payload[1] != 66 {
        return false
    }
    return true
}
...
func smoke_empty_tail_is_empty() bool {
    reply: service_effect.Effect = log_service.log_tail(log_service.log_init(21, 1))
    if service_effect.effect_reply_status(reply) != syscall.SyscallStatus.Ok {
        return false
    }
    if service_effect.effect_reply_payload_len(reply) != 0 {
        return false
    }
    return true
}
