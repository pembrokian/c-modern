...
void RunFreestandingKernelPhase106RealEchoServiceRequestReply(const std::filesystem::path& source_root,
    ...
    if (!run_outcome.exited || run_outcome.exit_code != 106) {
        Fail("phase106 freestanding kernel echo-service request-reply run should exit with the request-reply proof marker:\n" +
             run_output);
    }
