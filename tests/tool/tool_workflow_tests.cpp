...
int main(int argc, char** argv) {
	if (argc != 3 && argc != 4) {
		mc::test_support::Fail("expected source root, binary root, and optional workflow suite selector");
	}
	...
	if (case_name.empty()) {
		...
		mc::tool_tests::RunWorkflowToolSuiteCase(source_root, binary_root, mc_path, case_name);
	}
	return 0;
}