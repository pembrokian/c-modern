...
void TestKernelResetLanePhase178TopologyRestartProjectRuns(const std::filesystem::path& source_root,
                                                           const std::filesystem::path& binary_root,
                                                           const std::filesystem::path& mc_path) {
    const std::filesystem::path fixture_root = source_root / "tests" / "system" / "kernel_reset_lane_phase178_topology_restart";
    const std::filesystem::path project_root = binary_root / "kernel_reset_lane_phase178_topology_restart_project";
    const std::filesystem::path project_path =
        InstallKernelResetLaneFixtureProject(source_root, fixture_root, project_root);

    const std::filesystem::path build_dir = binary_root / "kernel_reset_lane_phase178_topology_restart_build";
    std::filesystem::remove_all(build_dir);
    BuildProjectTargetAndExpectSuccess(mc_path,
                                       project_path,
                                       build_dir,
                                       "app",
                                       "kernel_reset_lane_phase178_topology_restart_build_output.txt",
                                       "kernel reset lane phase 178 topology restart build");
    ...
    const auto [run_outcome, run_output] = RunCommandCapture({mc_path.generic_string(),
                                                              ...
                                                              "--build-dir",
                                                              build_dir.generic_string()},
                                                             build_dir / "kernel_reset_lane_phase178_topology_restart_run_output.txt",
                                                             "kernel reset lane phase 178 topology restart run");
    if (!run_outcome.exited || run_outcome.exit_code != 0) {
        Fail("kernel reset lane phase 178 topology restart project should return 0, got:\n" + run_output);
    }
}
...
void TestKernelResetLanePhase181KvRetainedReloadProjectRuns(const std::filesystem::path& source_root,
                                                            const std::filesystem::path& binary_root,
                                                            const std::filesystem::path& mc_path) {
    const std::filesystem::path fixture_root = source_root / "tests" / "system" / "kernel_reset_lane_phase181_kv_retained_reload";
    const std::filesystem::path project_root = binary_root / "kernel_reset_lane_phase181_kv_retained_reload_project";
    const std::filesystem::path project_path =
        InstallKernelResetLaneFixtureProject(source_root, fixture_root, project_root);

    const std::filesystem::path build_dir = binary_root / "kernel_reset_lane_phase181_kv_retained_reload_build";
    std::filesystem::remove_all(build_dir);
    BuildProjectTargetAndExpectSuccess(mc_path,
                                       project_path,
                                       build_dir,
                                       "app",
                                       "kernel_reset_lane_phase181_kv_retained_reload_build_output.txt",
                                       "kernel reset lane phase 181 kv retained reload build");
    ...
    const auto [run_outcome, run_output] = RunCommandCapture({mc_path.generic_string(),
                                                              ...
                                                              "--build-dir",
                                                              build_dir.generic_string()},
                                                             build_dir / "kernel_reset_lane_phase181_kv_retained_reload_run_output.txt",
                                                             "kernel reset lane phase 181 kv retained reload run");
    if (!run_outcome.exited || run_outcome.exit_code != 0) {
        Fail("kernel reset lane phase 181 kv retained reload project should return 0, got:\n" + run_output);
    }
}

void TestKernelResetLanePhase183EndpointAuthorityProjectRuns(const std::filesystem::path& source_root,
                                                             const std::filesystem::path& binary_root,
                                                             const std::filesystem::path& mc_path) {
    const std::filesystem::path fixture_root = source_root / "tests" / "system" / "kernel_reset_lane_phase183_endpoint_authority";
    const std::filesystem::path project_root = binary_root / "kernel_reset_lane_phase183_endpoint_authority_project";
    const std::filesystem::path project_path =
        InstallKernelResetLaneFixtureProject(source_root, fixture_root, project_root);

    const std::filesystem::path build_dir = binary_root / "kernel_reset_lane_phase183_endpoint_authority_build";
    std::filesystem::remove_all(build_dir);
    BuildProjectTargetAndExpectSuccess(mc_path,
                                       project_path,
                                       build_dir,
                                       "app",
                                       "kernel_reset_lane_phase183_endpoint_authority_build_output.txt",
                                       "kernel reset lane phase 183 endpoint authority build");
    ...
    const auto [run_outcome, run_output] = RunCommandCapture({mc_path.generic_string(),
                                                              ...
                                                              "--build-dir",
                                                              build_dir.generic_string()},
                                                             build_dir / "kernel_reset_lane_phase183_endpoint_authority_run_output.txt",
                                                             "kernel reset lane phase 183 endpoint authority run");
    if (!run_outcome.exited || run_outcome.exit_code != 0) {
        Fail("kernel reset lane phase 183 endpoint authority project should return 0, got:\n" + run_output);
    }
}
...
void RunWorkflowToolSuite(const std::filesystem::path& source_root,
    ...
    TestKernelResetLanePhase181KvRetainedReloadProjectRuns(source_root, suite_root, mc_path);
    TestKernelResetLanePhase183EndpointAuthorityProjectRuns(source_root, suite_root, mc_path);
}
