...
The test tree follows the repository validation layers:

- `parser/` for parser success dumps and parse-fail diagnostics
- `sema/` for semantic module dumps, semantic-fail diagnostics, and import or package fixtures
- `mir/` for MIR dumps, lowering-fail diagnostics, and canonical-example sync checks
- `codegen/` for deterministic backend dumps and executable behavior tests
- `stdlib/` for checked-in stdlib proof programs
- `tool/` for `mc` driver, manifest, rebuild, and workflow tests
...
- `tests/parser/` is autodiscovered by adjacent `.ast.txt` or `.errors.txt` files
- `tests/codegen/` backend dump fixtures are autodiscovered by adjacent `.backend.txt` files
- `tests/sema/` and `tests/mir/` use a hybrid model: ordinary adjacent `.sema.txt`/`.mir.txt` and `.errors.txt` fixtures are autodiscovered, while explicit runner metadata remains only for import roots, imported modules, canonical-example sync, or package-identity overrides
