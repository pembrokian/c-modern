...
- the admitted repository-bounded Phase 29 static-library slice with `examples/real/issue_rollup/`, proving one hosted `staticlib` target consumed by two thin executable targets and by ordinary `mc test` workflow through the same archive boundary
