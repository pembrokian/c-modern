...
- admitted service-layer boundary: shared service_effect Message and Effect contract; log_service, kv_service, queue_service, shell_service with compact hostile-user invalid classification (`?C`, `?S`), serial_service, echo_service, and transfer_service
- admitted topology boundary: service_topology.mc owns SERIAL, SHELL, LOG, KV, ECHO, TRANSFER, and QUEUE endpoint constants plus flat lookup helpers and static restart classification; shell_service.mc is topology-neutral
...
- admitted orchestration boundary: scenarios.mc owns the scripted observation loop; main.mc is init plus one call; init.mc owns endpoint-driven restart while retained-state reload now covers log, kv, and queue explicitly
- admitted proof set: Phases 109–187 (see `docs/plan/admin/veya_running_system_support_statement.txt` for the full list)
