...
- admitted topology boundary: service_topology.mc owns SERIAL, SHELL, LOG, KV, ECHO, and TRANSFER endpoint constants plus flat lookup helpers; shell_service.mc is topology-neutral
...
- admitted orchestration boundary: scenarios.mc owns the scripted observation loop; main.mc is init plus one call; init.mc owns endpoint-driven restart and log retained-state reload
- admitted proof set: Phases 109–185 (see `docs/plan/admin/veya_running_system_support_statement.txt` for the full list)
