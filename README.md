...
- the admitted repository-bounded hosted `os` subprocess slice with narrow `stdlib/os.mc` `spawn_argv(...)` plus `wait(...)`, the legacy fixed `spawn(...)` compatibility wrapper, explicit argv ownership, pipe-backed stdin/stdout redirection through ordinary `io.File` handles, and the real `examples/real/line_filter_relay/` project proving one bounded child-process line relay without shell semantics or broader process policy
...
- the admitted hosted `os` surface now includes explicit argv-slice `spawn_argv(...)`, the legacy fixed `spawn(...)` compatibility wrapper, and blocking `wait(...)` with bounded stdin/stdout pipe redirection through ordinary `io.File` handles; shell command strings, environment APIs, cwd mutation, stderr policy, signals, and broader process management remain deferred.
...
- known limitations remain explicit: the supported host target is still Darwin arm64 only, the admitted networking surface remains narrow IPv4 TCP plus poller support rather than a broader async or portability claim, and the admitted hosted `sync` slice still excludes public compare-exchange, exchange, fetch-add, schedulers, and broader portability claims.
