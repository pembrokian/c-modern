...
- admitted first human-facing app boundary: `issue_rollup` now runs as one directly usable installed foreground app proof over the admitted input/display path, with app-local interaction kept in the repository-owned example tree and visible state still bounded to the readable four-cell `EMTY`/`STDY`/`BUSY`/`ATTN` contract
