...
- admitted service-layer boundary: shared service_effect Message and Effect contract; log_service, kv_service, queue_service, ticket_service, journal_service as one bounded durable replay owner, shell_service with compact hostile-user invalid classification (`?C`, `?S`), serial_service, echo_service, and transfer_service
- admitted topology boundary: service_topology.mc owns SERIAL, SHELL, LOG, KV, ECHO, TRANSFER, QUEUE, TICKET, and JOURNAL endpoint constants plus flat lookup helpers and static restart classification; shell_service.mc is topology-neutral
...
- admitted proof set: Phases 109–230 (see `docs/plan/admin/veya_running_system_support_statement.txt` for the full list)
