...
- the admitted hosted `os` surface now includes only explicit argv-based `spawn(...)` plus blocking `wait(...)` with bounded stdin/stdout pipe redirection through ordinary `io.File` handles; shell command strings, environment APIs, cwd mutation, stderr policy, signals, and broader process management remain deferred.
