...
- The repository currently supports one bounded human-facing slice on Darwin arm64 when built from source: hosted `issue_rollup` and `review_board` stay on the ordinary `mc` project workflow, and the repository-owned reset-lane kernel supports one maintained small-system demo with one launcher-selected foreground app on one fixed four-cell visible surface, one key-only input family, one installed `issue_rollup` path, one direct-launch `review_board` path with additive detail-selection and key-only filter-edit interaction, and one bounded persisted `review_board` reload path that now preserves the active detail view plus one in-progress filter-edit state.
...
- the admitted repository-specific first-pass Phase 8 slice with canonical pressure-program proof plus bounded real utility projects for text search, file walking, whole-file hashing, and arena-backed parsing, now followed by the explicit Phase 263 scratch-work landing for maintained arena reuse
...
- the admitted repository-bounded fixed-size pool allocator slice with `stdlib/mem.mc` `mem.Pool`, hosted runtime backing, and the real `examples/real/pool_rows/` project proving explicit take, exhaustion, return, and reuse through the ordinary project `mc run` or `mc test` workflow without widening into allocator frameworks or compiler-visible typed sizing
- the admitted repository-bounded larger-granularity run allocation slice with page-rounded `stdlib/mem.mc` `mem.Run`, hosted runtime backing, and the real `examples/real/bundle_stage/` project proving explicit bundle-sized staging, used-byte tracking, page-rounded capacity, and release through the ordinary project `mc run` or `mc test` workflow without widening into VM, mapping, or caching frameworks
- the admitted repository-bounded first install-slot slice in the hosted reset-lane kernel, where `update_store_service` now owns one explicit installed-program slot for the fixed `issue_rollup` artifact and `launcher_service` consumes that truth through bounded identify plus launch gating without widening into package management, bundle directories, or a process-management framework
...
- the admitted repository-bounded freestanding proof slice with explicit `env = "freestanding"` executable targets, explicit target identity and startup selection, optional manifest-owned extra link inputs, narrow `hal` `mmio_ptr<T>`, `volatile_load<T>`, and `volatile_store<T>` support, one bounded endpoint-queue representation proof, one minimal task-lifecycle representation proof, one kernel static-data follow-through proof with ordinary top-level consts, globals, structs, and enums, one bounded kernel build-integration audit proving emitted objects compose with a minimal external relink step, one bounded init-to-log-service handshake proof for the first small user-space policy path, one bounded capability-handle transfer proof showing explicit user-space authority handoff and use, one bounded ordinary helper-module proof showing repeated transfer and request scaffolding can stay in user space without a widened runtime surface, one bounded user-space lifecycle proof showing explicit launch authority and exit observation can stay in ordinary user code, and one bounded timer wake proof showing one deterministic event source can wake one blocked worker without widening into a general interrupt or scheduler framework, proved by the Phase 81 boot-marker, Phase 82 explicit link-input, Phase 83 polling-console, Phase 85 kernel queue, Phase 86 task lifecycle, Phase 87 static-data, Phase 88 manual-link, Phase 89 init-to-log-service, Phase 90 capability-transfer, Phase 91 helper-boundary, Phase 92 user-space-lifecycle, and Phase 93 timer-wake tool regressions without claiming broad freestanding packaging or cross-compilation
...
- admitted richer real-project proof set: `examples/real/issue_rollup/`, `examples/real/worker_queue/`, `examples/real/pipe_handoff/`, `examples/real/pipe_ready/`, `examples/real/pool_rows/`, `examples/real/bundle_stage/`, `examples/real/line_filter_relay/`, and `examples/real/evented_echo/`, where `worker_queue` is the low-level shared-memory sync proof, `pipe_handoff` is the direct blocking handle-first communication proof, `pipe_ready` is the pipe-readiness proof over `io.poller_*`, `pool_rows` is the fixed-size pool take-or-return proof over `mem.Pool`, `bundle_stage` is the page-rounded larger-granularity staging proof over `mem.Run`, `line_filter_relay` is the narrow subprocess plus stdio proof over `os.spawn_pipe_argv(...)`, `os.spawn_pipe_argv_merged_output(...)`, `os.spawn_pipe_argv_split_output(...)`, and `os.wait(...)`, and `evented_echo` remains the stronger networking proof over the same handle model
...
- admitted kernel mechanism boundary: explicit boot entry, first user entry, endpoint and handle state, syscall-owned byte IPC, one explicit transferred-handle grant-and-consume path, program-cap spawn and wait, timer-backed wake, bootstrap-capability handoff, named dispatch arms, per-send delivery witness
- admitted service-layer boundary: shared service_effect Message and Effect contract; log_service, kv_service, queue_service, ticket_service, journal_service as one bounded durable replay owner, object_store_service as one bounded durable named-object owner, update_store_service as one bounded staged-update plus installed-slot owner, launcher_service as one bounded catalog-plus-installed-truth consumer, shell_service with compact hostile-user invalid classification (`?C`, `?S`), serial_service, echo_service, and transfer_service
- admitted small user-facing system boundary: the installed `issue_rollup` path, the direct-launch `review_board` path with additive `Summary` or `Open` or `Closed` or `Urgent` detail selection plus bounded key-only filter editing, and the bounded persisted `review_board` reload path now compose through the same launcher, update-store, display, input, and app owners as one maintained reset-lane small-system demo, while visible state stays bounded to the readable four-cell `EMTY`/`STDY`/`BUSY`/`ATTN` contract and the repository still does not claim a desktop shell, package manager, broader layout model, or app framework
- admitted topology boundary: service_topology.mc owns SERIAL, SHELL, LOG, KV, ECHO, TRANSFER, QUEUE, TICKET, JOURNAL, OBJECT_STORE, WORKFLOW, COMPLETION_MAILBOX, LEASE, CONNECTION, and UPDATE_STORE endpoint constants plus flat lookup helpers, static restart classification, authority classification, and one same-module compile-time `ServiceDescriptor` table carrying labels plus topology metadata; shell_service.mc is topology-neutral
...
- admitted orchestration boundary: scenarios.mc owns the scripted observation loop; main.mc is init plus one call; init.mc owns endpoint-driven restart, explicit retained-state reload for log or kv or queue, explicit durable reload for journal, explicit coordinated retained restart for the workset (`kv` plus `queue`) and audit lane (`kv` plus `log`), and boot-owned workset plus audit generation markers and last-restart outcomes visible through the shell identity or retained-summary path; service_state owns the compact retained-versus-durable boundary payload projected by `XD<target>!`
- admitted proof set: Phases 109–242 (see `docs/plan/admin/veya_running_system_support_statement.txt` for the full list)
...
- The integrated small-system demo now lives on that same maintained reset-lane path through `tests/system/kernel_reset_lane_phase290_integrated_small_user_facing_system_demo/`; use `mc_tool_workflow_kernel_reset_lane_unit` as the owning executable proof for the composed installed `issue_rollup` plus direct-launch `review_board` plus persisted `review_board` reload flow.
...
build/debug/mc run --project examples/real/pool_rows/build.toml --build-dir build/debug/probes/pool_rows
build/debug/mc test --project examples/real/pool_rows/build.toml --build-dir build/debug/probes/pool_rows
build/debug/mc run --project examples/real/bundle_stage/build.toml --build-dir build/debug/probes/bundle_stage -- path/to/bundle.bin
build/debug/mc test --project examples/real/bundle_stage/build.toml --build-dir build/debug/probes/bundle_stage
