...
- admitted small user-facing system boundary: the installed `issue_rollup` path, the direct-launch `review_board` path, and the bounded persisted `review_board` reload path now compose through the same launcher, update-store, display, input, and app owners as one maintained reset-lane small-system demo, while visible state stays bounded to the readable four-cell `EMTY`/`STDY`/`BUSY`/`ATTN` contract and the repository still does not claim a desktop shell, package manager, or app framework
