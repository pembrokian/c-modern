...
- Phase 258 spends one same-module app-local procedure table in `examples/real/issue_rollup/src/render/rollup_render.mc`, so render selection now uses one explicit `[4]func() i32` table while kernel dispatch, shell routing, and boot topology remain intentionally flat owners.
