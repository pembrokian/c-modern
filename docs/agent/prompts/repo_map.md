...
- `kernel`
  ...
  - `src/issue_rollup_app.mc`: explicit first app-owned present seam that keeps one bounded `issue_rollup` summary state, derives visible four-cell tokens from the `issue_rollup` render modules plus the installed manifest bytes, and requests `display_surface.display_present(...)` on launch or admitted key-driven state changes without widening launcher, display, or dispatch into a redraw framework
