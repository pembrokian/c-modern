...
- `kernel/`: repository-owned Canopus kernel bring-up tree; currently a Phase 114 address-space-and-mmu-ownership-bounded kernel target
...
- `kernel`
  ...
  - `src/main.mc`: explicit architecture entry plus thin root orchestration over the landed first-user-entry, endpoint core, syscall IPC, interrupt handoff, MMU-owned translation-root construction, service, scheduler, lifecycle, and debug audit owners through the Phase 114 address-space and MMU ownership split boundary
  ...
  - `src/debug.mc`: debug-owned Phase 108 image/program-cap audit, Phase 109 first-running-kernel-slice audit, Phase 110 ownership-split audit, Phase 111 lifecycle-ownership audit, Phase 112 syscall-boundary audit, Phase 113 interrupt-boundary audit, and Phase 114 address-space/MMU audit
  ...
  - `src/mmu.mc`: bounded translation-root construction and activation records for the landed first-user and spawn path
