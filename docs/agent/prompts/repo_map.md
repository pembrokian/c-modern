...
- `kernel/`: repository-owned Veya kernel bring-up tree; currently through the Phase 268 bounded launcher slice over the hosted reset-lane workflow surface, with the Phase 255 through Phase 260 Band A compiler follow-through wave landed on adjacent boot/helper, topology, render-table, and record-construction owners, the Phase 261 audit classifying Band B as the stronger next owner, and Phase 267 plus Phase 268 now opening Band C with explicit runtime program identity and one bounded launcher owner
...
- `kernel`
  ...
  - `src/services/launcher_service.mc`: bounded single-foreground launcher owner consuming the fixed `program_catalog` table, with explicit catalog listing, selection, and launch-one-foreground consequences while keeping install, restart-visible resume truth, and real execution policy deferred
