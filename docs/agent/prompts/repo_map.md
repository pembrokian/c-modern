...
- `kernel/`: repository-owned Canopus kernel bring-up tree; currently a Phase 101 program-cap spawn and wait kernel target
...
- `kernel`
  ...
  - `src/main.mc`: explicit architecture entry, bounded first-user-entry, bounded endpoint-and-handle-core validation, bounded syscall-byte-IPC validation, bounded capability-carrying transfer validation, and bounded spawn-and-wait validation
  ...
  - `src/syscall.mc`: bounded syscall gate, byte-plus-capability send-and-receive request, bounded spawn-and-wait request, and observation records
...
- `tests/tool`
  ...
  - grouped tool coverage now lives in:
    ...
    - `tests/tool/freestanding/kernel/phase85_endpoint_queue.cpp`, `phase86_task_lifecycle.cpp`, `phase87_static_data.cpp`, `phase88_build_integration.cpp`, `phase97_user_entry.cpp`, `phase98_endpoint_handle_core.cpp`, `phase101_spawn_wait.cpp`: one kernel proof per file
