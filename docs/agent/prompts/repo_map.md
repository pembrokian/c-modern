...
- `kernel/`: repository-owned Canopus kernel bring-up tree; currently a Phase 117 init-orchestrated-multi-service kernel target
...
- `kernel`
  ...
  - `src/main.mc`: explicit architecture entry plus thin root orchestration over the landed first-user-entry, endpoint core, syscall IPC, interrupt classification, timer-owned tick delivery, MMU-owned translation-root construction, bounded init-owned multi-service orchestration, scheduler, lifecycle, and debug audit owners through the Phase 117 init-orchestrated multi-service bring-up boundary
