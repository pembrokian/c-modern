...
- `kernel/`: repository-owned Canopus kernel bring-up tree; currently a Phase 97 address-space and first-user-entry kernel target
...
- `kernel`
  ...
  - `src/main.mc`: explicit architecture entry and bounded first-user-entry validation
...
- `tests/tool`
  ...
  - grouped tool coverage now lives in:
    ...
    - `tests/tool/freestanding/kernel/phase85_endpoint_queue.cpp`, `phase86_task_lifecycle.cpp`, `phase87_static_data.cpp`, `phase88_build_integration.cpp`, `phase97_user_entry.cpp`: one kernel proof per file
