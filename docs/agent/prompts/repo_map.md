...
- `kernel/`: repository-owned Canopus kernel bring-up tree; currently a Phase 135 buffer-ownership-boundary target
...
- `kernel`
  ...
  - `src/serial_service.mc`: bounded serial-service ingress state plus one service-owned copied receive-frame log
  ...
  - `src/uart.mc`: bounded UART receive-frame staging, copied publish, and deterministic retirement owner for the landed Phase 135 path
