...
- `kernel/`: repository-owned Veya kernel bring-up tree; currently at the Phase 244 delegated installer authority slice over the hosted reset-lane workflow surface
...
- `kernel`
  ...
  - `src/lease_service.mc`: bounded retained temporary delegation owner for one completion-bound workflow lease path, one delegated durable named-object update lease shape bound to existing restart-visible generation truth, one delegated external ticket-use lease shape bound to `ticket_service` generation truth, and one installer-apply lease shape bound to the retained update workflow stale boundary
