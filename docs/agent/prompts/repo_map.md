...
- `kernel/`: repository-owned Canopus kernel bring-up tree; currently a Phase 115 timer-ownership-hardened kernel target
...
- `kernel`
  ...
  - `src/main.mc`: explicit architecture entry plus thin root orchestration over the landed first-user-entry, endpoint core, syscall IPC, interrupt classification, timer-owned tick delivery, MMU-owned translation-root construction, service, scheduler, lifecycle, and debug audit owners through the Phase 115 timer ownership hardening boundary
