...
- Phase 275 admits one explicit `display_surface.mc` owner and spends it through one bounded launch-time handoff from `launcher_service` foreground truth to one fixed four-cell visible target. `display_surface.mc` owns the present/query contract, `program_catalog.mc` owns the tiny foreground display tokens, and `kernel_dispatch.mc` stays at one bounded routing handoff rather than absorbing composition or app render policy.
