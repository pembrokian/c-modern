...
- `kernel/`: repository-owned Canopus kernel bring-up tree; currently a Phase 136 device-failure-containment target
...
- `kernel`
  ...
  - `src/serial_service.mc`: bounded serial-service ingress state plus one service-owned copied receive-frame log and one service-local malformed-input classification path
  ...
  - `src/interrupt.mc`: bounded interrupt controller, architecture-entry records, and generic dispatch classification for the landed timer-backed wake path plus the bounded UART receive path
  - `src/uart.mc`: bounded UART receive-frame staging, copied publish, deterministic retirement, and bounded drop owner for the landed Phase 136 path
