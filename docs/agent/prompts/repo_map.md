...
- `kernel/`: repository-owned Canopus kernel bring-up tree; currently a Phase 134 minimal-device-service-handoff target
...
- `kernel`
  ...
  - `src/serial_service.mc`: bounded serial-service ingress state and received-byte publication records
  ...
  - `src/uart.mc`: bounded UART receive register, acknowledge, and interrupt-origin byte publication owner for the landed Phase 134 path
