...
- `stdlib/`: standard library modules; `stdlib/mem.mc` now owns the admitted fixed-size `mem.Pool` byte-slot contract from Phase 262, the explicit `Arena` init/reset/new/deinit scratch-work contract landed in Phase 263, and the page-rounded raw-byte `mem.Run` contract landed in Phase 264; Phase 265 then explicitly keeps slab-style small-object reuse deferred because the maintained consumers still fit pool, arena, and run ownership honestly, and Phase 266 closes Band B with explicit selection guidance for when to choose buffers, pools, arenas, and runs; Phase 297 then adds one tiny shared `active_region` owner with only `Body` or `Status` truth plus a toggle helper for the repeated cross-app active-region slice
- `kernel/`: repository-owned Veya kernel bring-up tree; currently through the completed Phase 308 post-second-app platform direction audit on the hosted reset-lane workflow surface, with the Phase 255 through Phase 260 Band A compiler follow-through wave landed on adjacent boot/helper, topology, render-table, and record-construction owners, the Phase 261 audit classifying Band B as the stronger next owner, Phase 267 through Phase 272 closing Band C with explicit runtime program identity, one bounded launcher owner, one fixed installed-program slot, one bounded fresh-versus-resumed-versus-invalidated launch classification, one issue-rollup-only admitted manifest family, and one explicit launcher-owned installed-workflow status query for the first `issue_rollup` demo path, Phase 273 classifying the first UI work as next, Phase 274 then admitting one explicit foreground-input owner for the tiny `I K <byte> !` event family, Phase 275 admitting one explicit fixed display owner, Phase 276 adding one explicit foreground-routing seam that forwards the admitted input family to exactly one launcher-selected foreground app, Phase 277 replacing the old launcher-time display token shortcut with one app-owned present request path that keeps render policy in the `issue_rollup` render modules and surface update semantics in `display_surface.mc`, Phase 278 aligning the hosted and reset-lane `issue_rollup` paths on the same readable four-cell visible contract instead of admitting a second rendering family, Phase 279 then moving the first direct app interaction model into the repository-owned `issue_rollup` app tree so the visible UI proof now runs through app-local text-plus-parse-plus-render logic instead of a kernel-local placeholder state machine, Phase 280 finally spending the launcher-owned restart classification on that same app path through one launch-only `FRSH` or `RSUM` or `INVD` overlay that clears on the first app-owned state change rather than widening launcher, update_store, or display into retained UI-state owners, Phase 281 then closing Band D as one deliberately narrow human-facing lane, Phase 282 then admitting one adjacent fixed two-region composition owner that spends a two-cell body plus two-cell status strip on the custom-manifest `issue_rollup` path without widening display into layout policy or launcher into UI coordination, Phase 283 then explicitly keeping alternate input deferred because no admitted foreground app yet needs more than the current key-event lane while one focused reject proof guards that non-key `I <event> <value> !` frames stay unsupported instead of smuggling in a second input family, Phase 284 then making active-region truth explicit on that same custom-manifest path by keeping focus app-local in `issue_rollup_app.mc` as one bounded `Body` or `Status` toggle rather than widening launcher, input route, or display into focus-policy owners, Phase 286 then admitting `review_board` as the second directly usable UI proof through one consumer-local `review_board_ui` state machine, one thin `review_board_app.mc` wrapper, one bounded launcher direct-launch path that leaves `update_store_service` issue-rollup-specific, one maintained reset-lane proof plus one narrow real-project proof, Phase 287 then reusing `journal_service.mc` for one bounded four-byte `review_board` view-state lane that reloads through boot and launch without introducing a UI-state framework, Phase 288 then documenting the canonical `mc` and grouped-test workflow for `issue_rollup` and `review_board`, Phase 289 then freezing post-Band-E "basic UI" as one launcher-selected foreground app on one fixed four-cell surface with one optional two-region composition seam, one key-only input path, two directly usable app-local proofs, and one bounded journal-backed persisted view-state slice rather than a desktop, widget, or session framework, Phase 290 then composing those admitted owners into one maintained small-system demo that stages the installed `issue_rollup` path, switches to direct-launch `review_board`, and verifies bounded `review_board` reload after reboot without widening launcher, update_store, display, or app ownership, Phase 291 then classifying post-UI compiler pressure as one bounded source-level byte-literal readability slice rather than a broader compiler wave, Phase 292 then freezing the post-demo kernel owner split by keeping convenience pressure on the current launcher, display, workflow, persistence, and app-local owners instead of widening boot, dispatch, or launcher into hidden framework roles, Phase 294 then choosing richer UI on the existing fixed-surface and launcher-selected substrate as the strongest next wave, Phase 295 then making `review_board` an explicit first-class consumer of that same launcher-selected foreground-input contract through its existing app-local owner and maintained reset-lane plus hosted proofs, Phase 296 then extending that same app-local owner with one additive `Summary` or `Open` or `Closed` or `Urgent` detail-selection lane on the fixed four-cell surface without widening display, launcher, composition, or journal ownership, Phase 297 then extracting only the repeated `Body` or `Status` active-region truth into the tiny shared `stdlib/active_region.mc` owner while keeping key semantics, display policy, and routing app-local, Phase 298 then adding one app-local `review_board_ui` filter-edit slice entered on the existing key-only lane with bounded two-byte tokens `AL`, `OP`, `CL`, or `UR`, hosted and reset-lane proofs, Phase 299 then aligning the hosted and reset-lane `review_board` interaction proofs on that richer contract without adding a new runner or persistence family, Phase 300 then extending the existing `review_board` journal lane so one in-progress filter-edit substate plus its underlying detail view survives restart without widening `journal_service.mc` or `launcher_service.mc`, Phase 301 then freezing Band G as one more expressive but still fixed-surface, key-only, app-local UI slice while keeping broader layout, alternate input, toolkit, and packaging pressure explicitly deferred and naming the byte-literal compiler slice as the strongest next blocker, Phases 306 through 307 then landing one bounded two-app installed-program story on the existing `update_store_service` plus `launcher_service` split, and Phase 308 finally recording that the admitted two-app install story fully spends current packaging pressure so durable file-like growth now outranks further packaging or networking as the next deferred branch
...
- `kernel`
  ...
  - `src/issue_rollup_app.mc`: explicit first app-owned present seam that now acts as a thin kernel-side wrapper over the repository-owned `issue_rollup` interactive app logic, derives visible four-cell tokens from the existing parse and render modules plus the installed manifest bytes, consumes the launcher-owned fresh-versus-resumed-versus-invalidated classification only as a launch-time overlay (`FRSH`, `RSUM`, `INVD`), and on the custom-manifest path now also owns one bounded app-local active-region model (`Body` or `Status`) over the fixed two-region composition seam while keeping default-manifest rendering, launch overlay policy, and focus truth app-local
...
- Phase 284 keeps focus truth app-local to `issue_rollup_app.mc`, reuses the existing key-input path to toggle one bounded `Body` or `Status` region on the custom-manifest composed surface, and explicitly keeps `foreground_input_route.mc`, `display_surface.mc`, and `launcher_service.mc` out of in-app focus policy.
