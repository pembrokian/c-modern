...
struct ChildExecutionObservations {
    snapshot: ChildExecutionSnapshot
    last_interrupt_kind: interrupt.InterruptDispatchKind
}

struct ChildExecutionRuntimeState {
    identity: state.IdentityConfig
    init_program_capability: capability.CapabilitySlot
    kernel: bootstrap_state.KernelExecState
    capabilities: bootstrap_state.CapabilityState
}

struct ChildExecutionResult {
    runtime: ChildExecutionRuntimeState
    observations: ChildExecutionObservations
    succeeded: u32
}
...
func child_execution_observations(snapshot: ChildExecutionSnapshot, last_interrupt_kind: interrupt.InterruptDispatchKind) ChildExecutionObservations {
    return ChildExecutionObservations{ snapshot: snapshot, last_interrupt_kind: last_interrupt_kind }
}

func child_execution_runtime_state(identity: state.IdentityConfig, init_program_capability: capability.CapabilitySlot, kernel: bootstrap_state.KernelExecState, capabilities: bootstrap_state.CapabilityState) ChildExecutionRuntimeState {
    return ChildExecutionRuntimeState{ identity: identity, init_program_capability: init_program_capability, kernel: kernel, capabilities: capabilities }
}

func child_execution_result(runtime: ChildExecutionRuntimeState, observations: ChildExecutionObservations, succeeded: u32) ChildExecutionResult {
    return ChildExecutionResult{ runtime: runtime, observations: observations, succeeded: succeeded }
}
...
func execute_program_cap_spawn_and_wait(config: ChildExecutionConfig, runtime: ChildExecutionRuntimeState, init_image: init.InitImage, observations: ChildExecutionObservations) ChildExecutionResult {
    next_runtime: ChildExecutionRuntimeState = runtime
    next_observations: ChildExecutionObservations = observations

    next_runtime.capabilities.wait_tables[1] = capability.wait_table_for_owner(config.init_pid)
    ...
    spawn_result: syscall.SpawnResult = syscall.perform_spawn(next_runtime.kernel.gate, next_runtime.init_program_capability, next_runtime.kernel.process_slots, next_runtime.kernel.task_slots, next_runtime.capabilities.wait_tables[1], init_image, spawn_request, config.child_pid, config.child_tid, config.child_asid, new_translation_root)
    next_runtime.kernel.gate = spawn_result.gate
    next_runtime.init_program_capability = spawn_result.program_capability
    next_runtime.kernel.process_slots = spawn_result.process_slots
    next_runtime.kernel.task_slots = spawn_result.task_slots
    next_runtime.capabilities.wait_tables[1] = spawn_result.wait_table
    next_observations.snapshot = child_execution_snapshot(new_translation_root, spawn_result.child_address_space, spawn_result.child_frame, spawn_result.observation, next_observations.snapshot.sleep_observation, next_observations.snapshot.timer_wake_observation, next_observations.snapshot.wake_ready_queue, next_observations.snapshot.pre_exit_wait_observation, next_observations.snapshot.exit_wait_observation)
    next_runtime.kernel.ready_queue = state.user_ready_queue(config.child_tid)
    if syscall.status_score(next_observations.snapshot.spawn_observation.status) != 2 {
        return child_execution_result(next_runtime, next_observations, 0)
    }

    sleep_result: syscall.SleepResult = syscall.perform_sleep(next_runtime.kernel.gate, next_runtime.kernel.task_slots, next_runtime.kernel.timer_state, syscall.build_sleep_request(config.child_task_slot, 1))
    next_runtime.kernel.gate = sleep_result.gate
    next_runtime.kernel.task_slots = sleep_result.task_slots
    next_runtime.kernel.timer_state = sleep_result.timer_state
    next_observations.snapshot = child_execution_snapshot(next_observations.snapshot.translation_root, next_observations.snapshot.address_space, next_observations.snapshot.user_frame, next_observations.snapshot.spawn_observation, sleep_result.observation, next_observations.snapshot.timer_wake_observation, next_observations.snapshot.wake_ready_queue, next_observations.snapshot.pre_exit_wait_observation, next_observations.snapshot.exit_wait_observation)
    next_runtime.kernel.ready_queue = state.empty_queue()
    if syscall.status_score(next_observations.snapshot.sleep_observation.status) != 4 {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if syscall.id_score(next_runtime.kernel.gate.last_id) != 32 {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if syscall.status_score(next_runtime.kernel.gate.last_status) != 4 {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if state.task_state_score(next_runtime.kernel.task_slots[2].state) != 8 {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if next_observations.snapshot.sleep_observation.task_id != config.child_tid {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if next_observations.snapshot.sleep_observation.deadline_tick != 1 {
        return child_execution_result(next_runtime, next_observations, 0)
    }

    interrupt_entry: interrupt.InterruptEntry = interrupt.arch_enter_interrupt(next_runtime.kernel.interrupts, config.timer_interrupt_vector, config.interrupt_actor)
    next_runtime.kernel.interrupts = interrupt_entry.controller
    dispatch_result: interrupt.InterruptDispatchResult = interrupt.dispatch_interrupt(interrupt_entry)
    next_runtime.kernel.interrupts = dispatch_result.controller
    next_observations.last_interrupt_kind = dispatch_result.kind
    delivery: timer.TimerInterruptDelivery = timer.deliver_interrupt_tick(next_runtime.kernel.timer_state, 1)
    next_runtime.kernel.timer_state = delivery.timer_state
    next_observations.snapshot = child_execution_snapshot(next_observations.snapshot.translation_root, next_observations.snapshot.address_space, next_observations.snapshot.user_frame, next_observations.snapshot.spawn_observation, next_observations.snapshot.sleep_observation, delivery.observation, next_observations.snapshot.wake_ready_queue, next_observations.snapshot.pre_exit_wait_observation, next_observations.snapshot.exit_wait_observation)
    wake_transition: lifecycle.TaskTransition = lifecycle.ready_task(next_runtime.kernel.task_slots, 2)
    next_runtime.kernel.task_slots = wake_transition.task_slots
    next_runtime.kernel.ready_queue = state.user_ready_queue(config.child_tid)
    next_observations.snapshot = child_execution_snapshot(next_observations.snapshot.translation_root, next_observations.snapshot.address_space, next_observations.snapshot.user_frame, next_observations.snapshot.spawn_observation, next_observations.snapshot.sleep_observation, next_observations.snapshot.timer_wake_observation, next_runtime.kernel.ready_queue, next_observations.snapshot.pre_exit_wait_observation, next_observations.snapshot.exit_wait_observation)
    if next_observations.snapshot.timer_wake_observation.task_id != config.child_tid {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if next_observations.snapshot.timer_wake_observation.deadline_tick != 1 {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if next_observations.snapshot.timer_wake_observation.wake_tick != 1 {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if next_observations.snapshot.timer_wake_observation.wake_count != 1 {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if interrupt.dispatch_kind_score(next_observations.last_interrupt_kind) != 2 {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if next_runtime.kernel.timer_state.monotonic_tick != 1 {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if next_runtime.kernel.timer_state.wake_count != 1 {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if next_runtime.kernel.timer_state.count != 0 {
        return child_execution_result(next_runtime, next_observations, 0)
    }
    if state.task_state_score(next_runtime.kernel.task_slots[2].state) != 4 {
        return child_execution_result(next_runtime, next_observations, 0)
    }

    pre_wait_result: syscall.WaitResult = syscall.perform_wait(next_runtime.kernel.gate, next_runtime.kernel.process_slots, next_runtime.kernel.task_slots, next_runtime.capabilities.wait_tables[1], syscall.build_wait_request(config.child_wait_handle_slot))
    next_runtime.kernel.gate = pre_wait_result.gate
    next_runtime.kernel.process_slots = pre_wait_result.process_slots
    next_runtime.kernel.task_slots = pre_wait_result.task_slots
    next_runtime.capabilities.wait_tables[1] = pre_wait_result.wait_table
    next_observations.snapshot = child_execution_snapshot(next_observations.snapshot.translation_root, next_observations.snapshot.address_space, next_observations.snapshot.user_frame, next_observations.snapshot.spawn_observation, next_observations.snapshot.sleep_observation, next_observations.snapshot.timer_wake_observation, next_observations.snapshot.wake_ready_queue, pre_wait_result.observation, next_observations.snapshot.exit_wait_observation)
    if syscall.status_score(next_observations.snapshot.pre_exit_wait_observation.status) != 4 {
        return child_execution_result(next_runtime, next_observations, 0)
    }

    next_runtime.kernel.process_slots = lifecycle.exit_process(next_runtime.kernel.process_slots, 2)
    next_runtime.kernel.task_slots = lifecycle.exit_task(next_runtime.kernel.task_slots, 2)
    next_runtime.capabilities.wait_tables[1] = capability.mark_wait_handle_exited(next_runtime.capabilities.wait_tables[1], config.child_pid, config.child_exit_code)
    next_runtime.kernel.ready_queue = state.empty_queue()

    wait_result: syscall.WaitResult = syscall.perform_wait(next_runtime.kernel.gate, next_runtime.kernel.process_slots, next_runtime.kernel.task_slots, next_runtime.capabilities.wait_tables[1], syscall.build_wait_request(config.child_wait_handle_slot))
    next_runtime.kernel.gate = wait_result.gate
    next_runtime.kernel.process_slots = wait_result.process_slots
    next_runtime.kernel.task_slots = wait_result.task_slots
    next_runtime.capabilities.wait_tables[1] = wait_result.wait_table
    next_observations.snapshot = child_execution_snapshot(next_observations.snapshot.translation_root, next_observations.snapshot.address_space, next_observations.snapshot.user_frame, next_observations.snapshot.spawn_observation, next_observations.snapshot.sleep_observation, next_observations.snapshot.timer_wake_observation, next_observations.snapshot.wake_ready_queue, next_observations.snapshot.pre_exit_wait_observation, wait_result.observation)
    next_runtime.kernel.ready_queue = state.empty_queue()
    if syscall.status_score(next_observations.snapshot.exit_wait_observation.status) != 2 {
        return child_execution_result(next_runtime, next_observations, 0)
    }

    return child_execution_result(next_runtime, next_observations, 1)
}
