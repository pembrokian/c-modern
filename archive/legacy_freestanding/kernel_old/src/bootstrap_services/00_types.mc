...
struct Phase152ControlledVariationWorkflowResult {
    varied_system: Phase150RebuiltSystemState
...
func log_service_snapshot(state: LogServiceExecutionState) LogServiceSnapshot {
    return LogServiceSnapshot{ program_capability: state.program_capability, address_space: state.child_address_space, user_frame: state.child_user_frame, state: state.service_state, spawn_observation: state.spawn_observation, receive_observation: state.receive_observation, ack_observation: state.ack_observation, handshake: state.handshake, wait_observation: state.wait_observation }
}
...
func echo_service_snapshot(state: EchoServiceExecutionState) EchoServiceSnapshot {
    return EchoServiceSnapshot{ program_capability: state.program_capability, address_space: state.child_address_space, user_frame: state.child_user_frame, state: state.service_state, spawn_observation: state.spawn_observation, receive_observation: state.receive_observation, reply_observation: state.reply_observation, exchange: state.exchange, wait_observation: state.wait_observation }
}
...
func transfer_service_snapshot(state: TransferServiceExecutionState) TransferServiceSnapshot {
    return TransferServiceSnapshot{ program_capability: state.program_capability, address_space: state.child_address_space, user_frame: state.child_user_frame, state: state.service_state, spawn_observation: state.spawn_observation, grant_observation: state.grant_observation, emit_observation: state.emit_observation, transfer: state.transfer, wait_observation: state.wait_observation }
}
...
func serial_service_execution_state(snapshot: SerialServiceSnapshot, runtime: ServiceRuntimeState) SerialServiceExecutionState {
    return SerialServiceExecutionState{ program_capability: snapshot.program_capability, gate: runtime.gate, process_slots: runtime.process_slots, task_slots: runtime.task_slots, init_handle_table: runtime.init_handle_table, child_handle_table: runtime.child_handle_table, wait_table: runtime.wait_table, endpoints: runtime.endpoints, init_image: runtime.init_image, child_address_space: snapshot.address_space, child_user_frame: snapshot.user_frame, service_state: snapshot.state, spawn_observation: snapshot.spawn_observation, receive_observation: snapshot.receive_observation, failure_observation: snapshot.failure_observation, wait_observation: snapshot.wait_observation, ready_queue: runtime.ready_queue }
}
...
func phase152_controlled_variation_workflow_result(varied_system: Phase150RebuiltSystemState, pre_restart_log_append_route: shell_service.ShellRoutingObservation, pre_restart_log_tail_route: shell_service.ShellRoutingObservation, pre_restart_kv_set_route: shell_service.ShellRoutingObservation, pre_restart_kv_get_route: shell_service.ShellRoutingObservation, post_restart_log_tail_route: shell_service.ShellRoutingObservation, post_restart_kv_get_route: shell_service.ShellRoutingObservation, pre_restart_log_retention: log_service.LogRetentionObservation, post_restart_log_retention: log_service.LogRetentionObservation, post_restart_kv_retention: kv_service.KvRetentionObservation, restart: init.ServiceRestartObservation, succeeded: u32) Phase152ControlledVariationWorkflowResult {
    return Phase152ControlledVariationWorkflowResult{ varied_system: varied_system, pre_restart_log_append_route: pre_restart_log_append_route, pre_restart_log_tail_route: pre_restart_log_tail_route, pre_restart_kv_set_route: pre_restart_kv_set_route, pre_restart_kv_get_route: pre_restart_kv_get_route, post_restart_log_tail_route: post_restart_log_tail_route, post_restart_kv_get_route: post_restart_kv_get_route, pre_restart_log_retention: pre_restart_log_retention, post_restart_log_retention: post_restart_log_retention, post_restart_kv_retention: post_restart_kv_retention, restart: restart, succeeded: succeeded }
}
