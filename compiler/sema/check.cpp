...
class Checker {
  ...
  private:
    ...
    bool IsConcreteInferredBindingType(const Type& type) const {
        ...
        return type.kind != Type::Kind::kNil;
    }

    void ReportInvalidInferredBindingType(const ExprTypeValue& value,
                                          std::string_view name) {
        if (IsUnknown(value.type)) {
            Report(value.span, "cannot infer type for local binding " + std::string(name) + "; add an explicit type annotation");
            return;
        }
        if (value.type.kind == Type::Kind::kNil) {
            Report(value.span, "cannot infer type for local binding " + std::string(name) + " from nil; add an explicit pointer or optional type");
            return;
        }
    }
    ...
    ConstExprEvaluator MakeConstExprEvaluator() {
        return ConstExprEvaluator(ConstExprEvaluator::Context {
            ...
            .combine_qualified_name = [this](const Expr& expr) {
